...
<div class="max-w-4xl mx-auto">
    <div class="mb-12">
        <a href="{{ route('seller.dashboard') }}" class="inline-flex items-center gap-2 text-sm font-medium text-(--color-muted) hover:text-(--color-primary) mb-8 transition-colors group">
        ...
        <h1 class="text-4xl font-serif mb-4 leading-tight">List a <span class="text-(--color-primary)">New Book</span></h1>
        <p class="text-(--color-muted) font-medium">Provide the details below to add a new title to your collection.</p>
    </div>

    <form action="{{ route('seller.books.store') }}" method="POST" enctype="multipart/form-data" class="space-y-12">
        @csrf
        ...
        <div class="bg-(--color-surface) p-8 rounded-3xl border border-(--color-border) shadow-sm space-y-8">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="space-y-2">
                    ...
                    <select name="type" id="type" required 
                        ...
                        <option value="physical" {{ old('type') == 'physical' ? 'selected' : '' }}>Physical Book</option>
                        <option value="digital" {{ old('type') == 'digital' ? 'selected' : '' }}>E-Book (Digital)</option>
                    </select>
                </div>
            </div>
            ...
            <div class="space-y-2">
                ...
                <textarea name="description" id="description" rows="5" 
                    class="w-full bg-(--color-bg) border border-(--color-border) rounded-2xl px-5 py-4 focus:outline-none focus:border-(--color-primary) transition-colors placeholder:text-(--color-accent) resize-none">{{ old('description') }}</textarea>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-12">
            <div class="bg-(--color-surface) p-8 rounded-3xl border border-(--color-border) shadow-sm space-y-8">
                ...
                <div class="space-y-2">
                    ...
                    <input type="number" name="stock" id="stock" value="{{ old('stock', 1) }}" required 
                        class="w-full bg-(--color-bg) border border-(--color-border) rounded-2xl px-5 py-4 focus:outline-none focus:border-(--color-primary) transition-colors">
                </div>
            </div>
            ...
            <div class="bg-(--color-surface) p-8 rounded-3xl border border-(--color-border) shadow-sm space-y-8">
                ...
                <div class="space-y-2">
                    <label class="block text-xs font-black uppercase tracking-widest text-(--color-muted) ml-1 mb-2">Cover Image</label>
                    <div id="upload_zone" class="group relative border-2 border-dashed border-(--color-border) rounded-3xl p-8 transition-all hover:border-(--color-primary)/30 bg-(--color-bg)/50 text-center cursor-pointer overflow-hidden">
                        ...
                        <div id="preview_container" class="hidden absolute inset-0 z-10 bg-(--color-surface)">
                            <img id="image_preview" src="#" alt="Preview" class="w-full h-full object-cover">
                            <div class="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity">
                                <span class="text-white text-xs font-black uppercase tracking-widest">Change Image</span>
                            </div>
                        </div>

                        <div id="upload_prompt" class="space-y-4">
                            <div class="mx-auto w-12 h-12 bg-(--color-primary)/10 text-(--color-primary) rounded-2xl flex items-center justify-center">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
                            </div>
                            <div>
                                <span class="text-sm font-bold block">Upload Cover</span>
                                <span class="text-xs text-(--color-muted)">JPG, PNG or WebP (Max 2MB)</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="selection_indicator" class="hidden flex items-center gap-2 mt-3 ml-1 text-emerald-600">
                        ...
                        <span class="text-[10px] font-black uppercase tracking-widest">Image Selected Successfully</span>
                    </div>
                ...
                <div id="digital_asset_section" class="hidden pt-8 border-t border-(--color-border) space-y-4 animate-slide-down">
                    <label class="block text-xs font-black uppercase tracking-widest text-(--color-muted) ml-1 mb-2 italic">Digital Asset (Required for eBooks)</label>
                    <div class="relative group">
                        ...
                        <div id="pdf_status_zone" class="bg-(--color-bg) border border-(--color-border) group-hover:border-(--color-primary)/50 rounded-2xl px-6 py-5 flex items-center justify-between transition-all">
                            <div class="flex items-center gap-4">
                                <div class="w-10 h-10 bg-red-500/10 text-red-500 rounded-xl flex items-center justify-center">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14.5 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7.5L14.5 2z"/><polyline points="14 2 14 8 20 8"/></svg>
                                </div>
                                <div>
                                    <p id="pdf_name" class="text-sm font-bold text-(--color-text)">Upload eBook PDF</p>
                                    <p id="pdf_info" class="text-[10px] text-(--color-muted) font-medium uppercase tracking-widest">Max File Size: 10MB</p>
                                </div>
                            </div>
                            <span class="text-[10px] font-black uppercase tracking-widest text-(--color-primary) group-hover:underline">Choose File</span>
                        </div>
                    </div>
                ...
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    ebookInput.addEventListener('change', function() {
                        if (this.files && this.files[0]) {
                            ...
                            pdfInfo.textContent = (file.size / (1024 * 1024)).toFixed(2) + ' MB • PDF Ready';
                            statusZone.classList.add('bg-emerald-500/10', 'border-emerald-500/20');
                        }
                    });
                });
            </script>
        ...
        <div class="flex items-center justify-end gap-6 pt-8">
            <a href="{{ route('seller.dashboard') }}" class="text-sm font-bold text-(--color-muted) hover:text-(--color-text) transition-colors">Discard Changes</a>
